...
- We include releases for the Cluster API release cadence, but also
  release when features are ready and it makes sense.
...
BareMetal Operator. This includes code both in the baremetal-operator
repository, *plus types from dependencies in public APIs*.  Types and
...
This repository contains two types of branches: the *master* branch
and *release-X* branches.
...
The standard Kubernetes comment commands should work in this
repository.  See [Prow](https://prow.k8s.io/command-help) for a
command reference.
