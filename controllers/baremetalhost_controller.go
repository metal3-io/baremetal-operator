...
func (r *BareMetalHostReconciler) Reconcile(request ctrl.Request) (result ctrl.Result, err error) {
	...
	if !r.hostHasStatus(host) {
		...
		if err == nil && objStatus != nil {
			...
			if errStatus != nil {
				return ctrl.Result{}, errors.Wrap(err, "could not restore status from annotation")
			}
		...
		if err == nil && objStatus != nil {
			...
			if errStatus != nil {
				return ctrl.Result{}, errors.Wrap(err, "could not delete status annotation")
			}
			return ctrl.Result{Requeue: true}, nil
		}
	}
	...
	if host.DeletionTimestamp.IsZero() && !r.hostHasFinalizer(host) {
		reqLogger.Info(
			"adding finalizer",
...
func (r *BareMetalHostReconciler) credentialsErrorResult(err error, request reconcile.Request, host *metal3v1alpha1.BareMetalHost) (ctrl.Result, error) {
	switch err.(type) {
...
func (r *BareMetalHostReconciler) buildAndValidateBMCCredentials(request reconcile.Request, host *metal3v1alpha1.BareMetalHost) (bmcCreds *bmc.Credentials, bmcCredsSecret *corev1.Secret, err error) {
...
func (r *BareMetalHostReconciler) getBMCSecretAndSetOwner(request reconcile.Request, host *metal3v1alpha1.BareMetalHost) (bmcCredsSecret *corev1.Secret, err error) {
...
func (r *BareMetalHostReconciler) hostHasFinalizer(host *metal3v1alpha1.BareMetalHost) bool {
...
func (r *BareMetalHostReconciler) saveHostStatus(host *metal3v1alpha1.BareMetalHost) error {
	...
	return r.Status().Update(context.Background(), host)
}

func (r *BareMetalHostReconciler) publishEvent(request reconcile.Request, event corev1.Event) {
	reqLogger := r.Log.WithValues("baremetalhost", request.NamespacedName)
...
func (r *BareMetalHostReconciler) setErrorCondition(request reconcile.Request, host *metal3v1alpha1.BareMetalHost, errType metal3v1alpha1.ErrorType, message string) (changed bool, err error) {
	reqLogger := r.Log.WithValues("baremetalhost", request.NamespacedName)
...
func (r *BareMetalHostReconciler) setBMCCredentialsSecretOwner(request reconcile.Request, host *metal3v1alpha1.BareMetalHost, secret *corev1.Secret) (err error) {
	reqLogger := r.Log.WithValues("baremetalhost", request.NamespacedName)
