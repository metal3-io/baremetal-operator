...
func (r *BareMetalHostReconciler) Reconcile(request ctrl.Request) (result ctrl.Result, err error) {
	...
	if !hostHasStatus(host) {
		reqLogger.Info("Fetching Status from Annotation")
		objStatus, err := getHostStatusFromAnnotation(host)
...
func (r *BareMetalHostReconciler) setErrorCondition(request reconcile.Request, host *metal3v1alpha1.BareMetalHost, errType metal3v1alpha1.ErrorType, message string) (err error) {
	reqLogger := r.Log.WithValues("baremetalhost", request.NamespacedName)
