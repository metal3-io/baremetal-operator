...
## 1\. Configuring BMO to Watch Specific Namespaces
...
## 2\. Create Roles and RoleBindings for each namespaces

When BMO operates in a namespace-scoped mode, we need to create `Roles` and
`Rolebindings` for each namespaces that we added in WATCH_NAMESPACE in env
variable controller-manager Deployment. (We need to remove cluster wide
...
## 3\. Current Restrictions and Known Issues
