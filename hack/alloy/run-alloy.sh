...
#                LOKI_URL             push endpoint (default: test collector)
#                ALLOY_JOB            'job' label       (default metal3ci)
#                ALLOY_PIPELINE_ID    'pipeline_id'     (default $JOB_NAME)
#                ALLOY_BUILD_NUMBER   'build_number'    (default $BUILD_NUMBER or 0)
#                BMC_PROTOCOL         'bmc_protocol'    (e2e matrix leg, may be empty)
...
# Endpoint defaults to the test collector; override via env for test/live.
export LOKI_URL="${LOKI_URL:-https://log.apps.test.metal3.io/store/api/v1/push}"

# Labels default to Jenkins built-ins so no extra pipeline plumbing is required:
# JOB_NAME and BUILD_NUMBER are set by Jenkins; BMC_PROTOCOL by the e2e matrix.
export ALLOY_JOB="${ALLOY_JOB:-metal3ci}"
# pipeline_id must match the log-collector dashboard filter, which the Jenkins
# jobs build as var-pipeline=${JOB_NAME}. Fall back to a readable name locally.
export ALLOY_PIPELINE_ID="${ALLOY_PIPELINE_ID:-${JOB_NAME:-metal3-bmo-e2e}}"
export ALLOY_BUILD_NUMBER="${ALLOY_BUILD_NUMBER:-${BUILD_NUMBER:-0}}"
# Matrix-leg discriminator: both protocol cells share JOB_NAME + BUILD_NUMBER,
# so the protocol is carried as its own label rather than folded into pipeline_id.
export BMC_PROTOCOL="${BMC_PROTOCOL:-}"
