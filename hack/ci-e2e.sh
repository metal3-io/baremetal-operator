...
if [[ "${GINKGO_FOCUS}" == "basic" ]]; then
    # This IP address is used to reach virtual BMCs. The basic test is using
    # irso, and vbmc container is running on host, so it can be reached through
    # the docker network from kind node. Hence we need to configure vbmc to
    # listen to docker network IP address instead of metal3 interface IP.
    # Only relevant with ipmi, redfish will listen to every interface.
