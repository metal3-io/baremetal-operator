...
# NOTE(nuhakala): ironic.yaml has already been once envsubstituted when
# injecting IP addresses, but the SSH key was not injected. Hence, first move
# the file under temp so that we can create it again, now fully substituted.
# Doing in-place substitution can be buggy using shell redirection/pipe.
mv "${REPO_ROOT}/test/e2e/data/ironic-standalone-operator/ironic/base/ironic.yaml" /tmp/ironic-base.yaml.tmpl
# shellcheck disable=SC2016
SSH_PUB_KEY_CONTENT="${pub_ssh_key}" envsubst '${SSH_PUB_KEY_CONTENT}' < \
  "/tmp/ironic-base.yaml.tmpl" > \
