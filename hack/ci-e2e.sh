...
# URL and checksum consumed by the E2E tests (via e2eConfig.GetVariable).
export IMAGE_CHECKSUM="${CIRROS_SHA256}"
...
CIRROS_URI="https://download.cirros-cloud.net/${CIRROS_VERSION}/${IMAGE_FILE}"
ISO_URI="https://sourceforge.net/projects/systemrescuecd/files/sysresccd-x86/${SYSRESCUE_VERSION}/${ISO_FILE}/download"

if [[ ! -f "${IMAGE_DIR}/${IMAGE_FILE}" ]]; then
    safe_curl -o "${IMAGE_DIR}/${IMAGE_FILE}" "${CIRROS_URI}"
...
if [[ ! -f "${IMAGE_DIR}/${ISO_FILE}" ]]; then
    safe_curl -o "${IMAGE_DIR}/${ISO_FILE}" "${ISO_URI}"
...
if [[ ! -f "${IMAGE_DIR}/${IPA_FILE}" ]]; then
  safe_curl -o "${IMAGE_DIR}/${IPA_FILE}" "${IPA_BASEURI}/${IPA_FILE}"
fi
...
BMO_OVERLAYS=(
  ...
  "${REPO_ROOT}/config/overlays/e2e-release-0.14"
)
