...
func (r *BareMetalHostReconciler) Reconcile(ctx context.Context, request ctrl.Request) (result ctrl.Result, err error) {
	...
	if actResult.Dirty() {
		...
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("failed to save host status after %q: %w",
				initialState, err)
		}
	...
	if info.host.Name != "" {
		if !info.host.DeletionTimestamp.IsZero() {
			...
			if err = r.createHostFirmwareSettings(info); err != nil {
				...
				return actionError{fmt.Errorf("failed to create hostFirmwareSettings: %w", err)}
			}
			if supportsFirmwareComponents {
				if err = r.createHostFirmwareComponents(info); err != nil {
					...
					return actionError{fmt.Errorf("failed to create hostFirmwareComponents: %w", err)}
				}
			}
