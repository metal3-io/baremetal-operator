...
// handleDataImageBeforeDeprovisioining detaches the dataImage, before deprovisioning
// the host, and handles the power changes that are required for detaching.
func (r *BareMetalHostReconciler) handleDataImageBeforeDeprovisioining(prov provisioner.Provisioner, info *reconcileInfo, dataImage *metal3api.DataImage) actionResult {
	...
	// TODO (hroyrh) : the status field here might get briefly out of sync, find
	// the actual value instead
	// Check the current status and save it before trying to update it.
	// Or should we simply check info.host.Status.PoweredOn
...
func (r *BareMetalHostReconciler) actionDeprovisioning(prov provisioner.Provisioner, info *reconcileInfo) actionResult {
	...
	// WIP (hroyrh) : detach dataImage before deprovisioning
	...
	if !k8serrors.IsNotFound(err) {
		...
		// Get vmedia api here ?
		if dataImage.Status.AttachedImage.URL != "" {
			actionResult := r.handleDataImageBeforeDeprovisioining(prov, info, dataImage)
			if actionResult != nil {
				return actionResult
			}
		}
