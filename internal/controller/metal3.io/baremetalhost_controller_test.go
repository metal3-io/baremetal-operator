...
func TestInspectLLDPData(t *testing.T) {
	...
	// Verify that the BMH status has hardware details but no LLDP data
	...
	for i, nic := range host.Status.HardwareDetails.NIC {
		assert.Nil(t, nic.LLDP, "LLDP data should be nil in BMH status for NIC %d", i)
	}
