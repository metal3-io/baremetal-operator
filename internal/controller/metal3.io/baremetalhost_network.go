...
// preparePortConfigs resolves and validates the network interfaces for a host
...
func (r *BareMetalHostReconciler) preparePortConfigs(ctx context.Context, info *reconcileInfo) (bool, error) {
	if len(info.host.Spec.NetworkInterfaces) == 0 {
		...
		if len(info.host.Status.AppliedNetworkAttachmentConfigs) > 0 {
			// A non-nil empty map tells the provisioner to clear any
			// previously applied port configurations.
			info.portConfigs = make(map[string]*provisioner.PortConfig)
		}
		return dirty, nil
	}

	portConfigs, err := r.resolvePortConfigs(ctx, info.host, nicsFromInfo(info))
	if err != nil {
		return false, fmt.Errorf("failed to resolve port configs: %w", err)
	}

	dirty, err := r.validateNetworkInterfaces(ctx, info.host, hardwareDetailsFromInfo(info))
	if err != nil {
		return false, err
	}
...
// buildAppliedNetworkAttachmentConfigs builds the applied status from the resolved
// configs map and the host's network interfaces.
func buildAppliedNetworkAttachmentConfigs(info *reconcileInfo) []metal3api.AppliedNetworkAttachmentConfig {
	macToName := buildNICMacToNameMap(nicsFromInfo(info))
	applied := make([]metal3api.AppliedNetworkAttachmentConfig, 0, len(info.portConfigs))
	...
	for mac, config := range info.portConfigs {
		...
		appliedConfig := metal3api.AppliedNetworkAttachmentConfig{
			Name: name,
		...
		if config.SwitchPortIdentifier != nil {
			appliedConfig.SwitchPort = config.SwitchPortIdentifier
		}
		applied = append(applied, appliedConfig)
	}
...
func (r *BareMetalHostReconciler) setNetworkInterfaceValidation(host *metal3api.BareMetalHost, status metav1.ConditionStatus, reason, message string) (bool, error) {
	...
	if existing != nil && existing.Status == status && existing.ObservedGeneration == host.Generation &&
...
func (r *BareMetalHostReconciler) resolveSwitchPortIdentifier(netIf *metal3api.NetworkInterface, config *provisioner.PortConfig) {
...
func (r *BareMetalHostReconciler) resolvePortConfigs(ctx context.Context, host *metal3api.BareMetalHost, nics []metal3api.NIC) (map[string]*provisioner.PortConfig, error) {
	...
	for _, netIf := range host.Spec.NetworkInterfaces {
		...
		r.resolveSwitchPortIdentifier(&netIf, config)
