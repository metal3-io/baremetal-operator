...
The `/callback/` route authenticates with a token derived from the BMC password.
The `/serve/` and `/ks/` routes do not, because anaconda cannot present a
credential when it fetches a kickstart.

- `/serve/{uid}/{id}` is guarded only by the unguessable BareMetalHost UID.
- `/ks/{id}` identifies the caller from the MAC headers anaconda sends, which
  anything on the network can set, so it is spoofable by design.

Treat the provisioning network as the trust boundary, and keep secrets out of
`serve_register(vars=...)`, since anything placed there is rendered into a
response those two routes will hand out.
...
never `Provisioned`. Collection is best effort, so a BMC that serves only part
of the inventory yields a partial one rather than a failure.
