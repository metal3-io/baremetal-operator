...
type CallbackConfig struct {
	...
	TLSCert string
	TLSKey  string
	// Fallback kickstart served when no host matches, empty uses the compiled in one.
	KSFallbackConfigMap string
	KSFallbackKey       string
}
...
// fallback, KubeHostResolver.Namespace prefers the value the host supplies.
func PodNamespace() string { return os.Getenv("POD_NAMESPACE") }

// NamespaceResolver reports the namespace the plugin's own objects live in.
...
func LoadCallbackConfig() CallbackConfig {
	return CallbackConfig{
		...
		KSFallbackConfigMap: os.Getenv("STARLARK_KS_FALLBACK_CONFIGMAP"),
		KSFallbackKey:       cmp.Or(os.Getenv("STARLARK_KS_FALLBACK_KEY"), "ks.cfg"),
	}
}
...
type PluginServer struct {
	...
	Serve    ServeResolver
	Log      logr.Logger
	// Namespace holds the ConfigMaps the serve and kickstart routes read.
	Namespace string
}
