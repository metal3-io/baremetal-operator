...
func (r *KubeHostResolver) mutateHostSecret(ctx context.Context, namespace, name string, mutate func(data map[string][]byte) error) error {
	...
	for range hostSecretMaxAttempts {
		...
		if len(data) == 0 {
			...
			if err := r.Client.Delete(ctx, sec); err != nil {
				if k8serrors.IsNotFound(err) {
					return nil
				}
...
// ConfigMap or the key is absent. ConfigMaps are cached, so the plain client reads them.
...
// WriteServe upserts one route into the serve key of the host Secret.
...
// ReadServe returns a route registration and its namespace, found by BMH UID.
func (r *KubeHostResolver) ReadServe(ctx context.Context, uid, id string) (starlib.ServeRegistration, string, bool, error) {
	if r.Client == nil {
		return starlib.ServeRegistration{}, "", false, errors.New("serve read requires a Kubernetes client")
	}

	list := &corev1.SecretList{}
	if err := r.Client.List(ctx, list, client.MatchingLabels{ServeUIDLabel: uid}); err != nil {
	...
	for i := range list.Items {
		...
		if reg, ok := regs[id]; ok {
			return reg, sec.Namespace, true, nil
		}
	}
