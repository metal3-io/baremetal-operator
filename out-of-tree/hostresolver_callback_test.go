...
func TestHostSecretShared(t *testing.T) {
	...
	if _, found, _ := r.ReadServe(ctx, "bmh-uid", "ks"); !found {
...
func TestServeStoreRoundTrip(t *testing.T) {
	...
	reg, found, err := r.ReadServe(ctx, "bmh-uid", "ks")
	if err != nil || !found || reg.ConfigMap != "ks-cm" || reg.Key != "ks.cfg" || !reg.Render || reg.Vars["disk"] != "/dev/sda" {
		t.Fatalf("ReadServe = (%+v, %v, %v)", reg, found, err)
	}
	...
	if _, found, _ := r.ReadServe(ctx, "bmh-uid", "post"); !found {
...
func TestReadServeMissing(t *testing.T) {
	...
	if _, found, err := r.ReadServe(context.Background(), "no-uid", "ks"); err != nil || found {
...
// Scripts upsert the same registration on every reconcile, so an unchanged write
// must not churn the API server. A bumped ResourceVersion means it did.
func TestWriteServeSkipsUnchangedWrite(t *testing.T) {
	host := hostWithBMC("ns", "node-1", "bmc-creds")
	...
	if err := r.WriteServe(ctx, "ns", "node-1", "ks", reg); err != nil {
		t.Fatalf("first WriteServe: %v", err)
	}

	first := &corev1.Secret{}
	key := types.NamespacedName{Namespace: "ns", Name: starlib.HostSecretName("node-1")}
	if err := r.Client.Get(ctx, key, first); err != nil {
	...
	if err := r.Client.Get(ctx, key, second); err != nil {
		t.Fatalf("get after second write: %v", err)
	}

	if second.ResourceVersion != first.ResourceVersion {
		t.Errorf("ResourceVersion moved %s -> %s, the unchanged write was not skipped",
	...
	reg.Key = "other.cfg"
	if err := r.WriteServe(ctx, "ns", "node-1", "ks", reg); err != nil {
		t.Fatalf("changed WriteServe: %v", err)
	}

	third := &corev1.Secret{}
	if err := r.Client.Get(ctx, key, third); err != nil {
		t.Fatalf("get after changed write: %v", err)
	}

	if third.ResourceVersion == first.ResourceVersion {
		t.Error("a changed registration was skipped as well")
	}
}
