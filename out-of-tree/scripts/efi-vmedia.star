...
# Accepted Image.checksumType values (empty or "auto" means Ironic detects).
SUPPORTED_CHECKSUM_TYPES = ["", "auto", "md5", "sha256", "sha512"]

# BMO RootDeviceHints JSON tag maps to Ironic root_device key (only serial/wwn supported).
...
def parse_bmc_address(bmc_addr):
    """Parses redfish-virtualmedia+scheme://host:port/path into driver_info fields."""
    addr = bmc_addr

    # The +http or +https suffix picks the transport, matching the BMO bmc package.
    transport = "https"
    scheme_end = addr.find("://")
    if scheme_end >= 0:
        parts = addr[:scheme_end].split("+")
        if len(parts) > 1:
            transport = parts[1]
    ...
    out = {"redfish_address": transport + "://" + host_port}

    # Ironic derives the system itself when the address carries no path.
    if system_id and system_id.strip("/") not in ("", "redfish/v1"):
        out["redfish_system_id"] = system_id
    return out
...
def create_node(bmc_addr, bmc_user, bmc_password, boot_mac, data, verify_ca):
    """Creates a new node in Ironic via POST /v1/nodes."""
    # redfish_system_id is only present when the address carries a system path,
    # so merge the parsed map rather than indexing keys that may be absent.
    driver_info = {
    ...
    driver_info.update(parse_bmc_address(bmc_addr))
...
def build_register_patch(node, data, creds_changed, bmc):
    """Builds JSON-Patch operations for node configuration during register."""
    ops = []

    # Ironic never returns the password, so rewrite driver_info whenever anything
    # it cannot be diffed against changed.
    current_di = node.get("driver_info", {})
    addr_changed = current_di.get("redfish_address", "") != bmc["redfish_address"]
    verify_changed = current_di.get("redfish_verify_ca") != bmc.get("redfish_verify_ca")
    if creds_changed or addr_changed or verify_changed:
        for k, v in bmc.items():
            ops.append(patch_op("add", "/driver_info/" + k, v))

        # An address that lost its system path must not keep addressing the old one.
        if "redfish_system_id" not in bmc and "redfish_system_id" in current_di:
            ops.append(patch_op("remove", "/driver_info/redfish_system_id"))
...
def require_redfish_virtualmedia(host):
    ...
    # redfish virtualmedia with +http or +https normalize to the same Ironic driver.
    base = scheme.split("+")[0]
...
# The sub resource controllers build HostData with BuildHostDataNoBMC, so the
# methods they call get no BMC address and cannot check the scheme.
...
def register(host, data, creds_changed, _restart_on_failure):
    ...
    verify_ca = not host.get(HOST_DISABLE_CERT_VERIFY, False)
    node = None
    ...
    if not node:
        ...
        node, status = create_node(bmc_addr, bmc_user, bmc_password, boot_mac, data, verify_ca)
        if not node:
            if status == 409:
    ...
    bmc_di = parse_bmc_address(bmc_addr)
    bmc_di["redfish_username"] = bmc_user
    bmc_di["redfish_password"] = bmc_password
    bmc_di["redfish_verify_ca"] = verify_ca

    ops = build_register_patch(node, data, creds_changed, bmc_di)
    if ops:
...
def update_hardware_state(host):
    ...
    node, status = http_get("/v1/nodes/" + prov_id)
    if status != 200 or not node:
        return {"error": "update_hardware_state: cannot get node"}
...
def adopt(host, data, restart_on_failure):
    ...
    if state == MANAGEABLE:
        # While deprovisioning, leave Manageable alone and Deprovision will progress it.
...
def provision(host, data, force_reboot):
    ...
    # Deploy mode. CustomDeploy.method yields custom agent, else Image.url yields direct.
    ...
    if has_custom_deploy:
        # Custom deploy branch. Custom agent interface with user defined deploy step.
        if image_url:
        ...
        # Clear stale deploy_interface left by a prior ramdisk or custom agent attempt.
    ...
    if s == 409:
        ...
        return {"dirty": True, "requeue_after_seconds": REQUEUE_DELAY}
    if s < 200 or s >= 300:
        return {"dirty": True, "error": "provision: deploy request failed with status " + str(s)}
...
def power_on(host, _force):
    ...
    if s == 409:
        return {"dirty": True, "requeue_after_seconds": REQUEUE_DELAY}
    if s < 200 or s >= 300:
        return {"dirty": True, "error": "power_on: request failed with status " + str(s)}
...
def power_off(host, reboot_mode, force, _automated_cleaning_mode):
    ...
    if reboot_mode == "soft" and not force:
        ...
        if s == 400:
            ...
            _, s = change_power_state(prov_id, POWER_OFF)
    else:
    ...
    if s == 409:
        return {"dirty": True, "requeue_after_seconds": REQUEUE_DELAY}
    if s < 200 or s >= 300:
        # Without an error the controller never escalates soft to hard.
        return {"dirty": True, "error": "power_off: request failed with status " + str(s)}
