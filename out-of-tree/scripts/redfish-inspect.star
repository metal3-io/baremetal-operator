...
Inspects a BareMetalHost through the BMC without booting a ramdisk, and
controls power, health and virtual media over the same session. There is no
Ironic dependency, so a host reaches Available but cannot be Provisioned.
OS deployment, cleaning, RAID and firmware settings all report unsupported.
...
# Route id the kickstart is registered under. It must match the path in the
# inst.ks URL baked into the installer ISO.
KICKSTART_ID = "kickstart"
KICKSTART_CONFIGMAP = getenv("KICKSTART_CONFIGMAP") or "kickstart"
KICKSTART_KEY = getenv("KICKSTART_KEY") or "ks.cfg"
...
def has_bmc_address(host):
    """Report whether the host carries a BMC address at all, without aborting."""
    return bool(host.get(HOST_BMC_ADDRESS, "")) if host else False
...
def register_kickstart(host):
    """Register this host's kickstart route so the MAC keyed lookup can find it."""
    if not serve_enabled():
        # The env var only requests the listener, the bind can still have failed.
        return

    om = host.get(HOST_OBJECT_META, {}) or {}
    serve_register(
        id = KICKSTART_ID,
        configmap = KICKSTART_CONFIGMAP,
        key = KICKSTART_KEY,
        render = True,
        vars = {
            "Name": om.get("name", ""),
            "Namespace": om.get("namespace", ""),
            "UID": om.get("uid", ""),
        },
    )
...
def register(host, data, creds_changed, _restart_on_failure):
    ...
    # Upserted every register because deprovision and delete clear the serve routes.
    # WriteServe skips the write when the registration is unchanged.
    register_kickstart(host)
...
def power_off(host, reboot_mode, force, _automated_cleaning_mode):
    ...
    if not has_bmc_address(host):
        # Runs before deletion, so refusing here would wedge the finalizer.
        log_error("power_off: no BMC address, treating the host as off")
        return {}
...
def get_data_image_status(host):
    ...
    if not has_bmc_address(host):
        # The DataImage controller supplies no BMC, and refusing would strand the CR.
...
def attach_data_image(host, url):
    ...
    if not has_bmc_address(host):
        log_error("attach_data_image: no BMC address, cannot insert media")
        return None
...
def detach_data_image(host):
    ...
    if not has_bmc_address(host):
        log_error("detach_data_image: no BMC address, cannot eject media")
        return None
