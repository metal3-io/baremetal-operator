...
func BuiltinCallbackURL(thread *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, _ []starlark.Tuple) (starlark.Value, error) {
	...
	if cb.BaseURL == "" {
		return starlark.None, errors.New("callback_url: no base URL configured")
	}

	ns, _ := thread.Local(HostNamespaceThreadLocal).(string)
	host, _ := thread.Local(HostNameThreadLocal).(string)
	if ns == "" || host == "" {
		return starlark.None, errors.New("callback_url: BMH coordinates not set on thread")
	}

	return starlark.String(strings.TrimRight(cb.BaseURL, "/") + "/callback/" + ns + "/" + host), nil
}
