...
func serveThread(store ServeStore, baseURL, uid string) *starlark.Thread {
	thread := &starlark.Thread{Name: "t"}
	thread.SetLocal(ServeThreadLocal, &ServeContext{BaseURL: baseURL, UID: uid})
...
func TestBuiltinServeRegisterAndURL(t *testing.T) {
	...
	thread := serveThread(store, "http://cb:9080/", "uid-1")
...
func TestBuiltinServeRegisterVars(t *testing.T) {
	store := &stubServeStore{}
	thread := serveThread(store, "http://cb/", "uid-1")
