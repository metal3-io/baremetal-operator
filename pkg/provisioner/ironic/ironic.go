...
func (p *ironicProvisioner) GetDataImageStatus() (isImageAttached bool, err error) {
	...
	if isNodeBusy {
		return false, fmt.Errorf("node is busy, retry GetVirtualMedia request in some time")
	}

	err = nodes.GetVirtualMedia(p.ctx, p.client, p.nodeID).Err
	if err != nil {
		return false, err
	}

	body := nodes.GetVirtualMedia(p.ctx, p.client, p.nodeID).Body
