...
// Healthy reports only whether the BMC answers. The service root needs no
// credentials, so a rejected password cannot make a reachable BMC look down.
func (c Conn) Healthy(ctx context.Context) (bool, error) {
	...
	// The endpoint is BMO's own parse of spec.bmc.address, reaching it is the job.
	//nolint:gosec // G704, the BMC address is operator supplied by design
