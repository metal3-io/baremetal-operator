...
func CreateVolumePool(poolName string) (*libvirt.StoragePool, error) {
	...
	poolPath := "/tmp/pool"
	if err = os.Mkdir(poolPath, 0750); err != nil && !os.IsExist(err) {
...
func CreateVolume(volumeName, poolName string, capacityInGB int) error {
	// Connect to libvirt daemon
	conn, err := libvirt.NewConnect("qemu:///system")
	if err != nil {
		fmt.Println("Failed to connect to qemu:///system")
	...
	pool, err := CreateVolumePool(poolName)
...
func CreateLibvirtVM(hostName, networkName, macAddress string) error {
	...
	if err = CreateVolume(hostName, poolName, 20); err != nil {
		return err
	}
