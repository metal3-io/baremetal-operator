...
testing is not supported with fixture tests. IrSO version 0.10 is minimum
requirement for the tests. At the time of writing, this also requires using IPA
image which is built with `DIB_SIMPLE_INIT_NO_DHCP_FALLBACK=1`. This is not the
case with the upstream IPA image.
...
_Please note_ that the configuration files with IP addresses are created only
the first time when the tests are run. If you want to change the IP version you
need to remove the configuration files. This is automatically done by e2e
cleanup:
