...
const (
	// Stamped when the ISO goes into the drive. BeginInstall clears any recorded
	// verdict just before it, so a callback posted earlier is thrown away.
...
// anacondaKickstart is served and never run, the e2e ISO is not an installer.
// It proves the render, so it may only name variables the plugin passes.
...
// process runs outside it, which is how metrics_service_test.go reaches in too.
...
var _ = Describe("Anaconda", Label("anaconda"), func() {
	...
	BeforeEach(func() {
		...
		// This spec only means anything against a BMO running the plugin. Read
		// the map, GetVariable fails the spec when the key is absent.
	...
	It("should serve a kickstart, finish on the callback and then deprovision", func() {
		...
		Eventually(func(g Gomega) {
			...
			// A refused provision leaves the state at provisioning and records
			// the reason on the host. Without it a timeout here explains nothing.
			g.Expect(provisioned.Annotations).To(HaveKey(installStartedAnnotation),
				fmt.Sprintf("the plugin has not stamped the install start. operationalStatus %q, errorMessage %q",
					provisioned.Status.OperationalStatus, provisioned.Status.ErrorMessage))
		...
		// Only a deviceName hint renders to something predictable. A wwn or serial
		// becomes a by-id path, and an empty expectation would pass against all.
		...
		// Deprovision ejects the media and erases nothing, so BMO leaves
		// deprovisioning in the same reconcile. Only available is observable.
		...
		// Available alone is also where a host that never provisioned sits.
		// Dropping the stamp is what says teardown actually ran.
		By("Checking that teardown cleared the install state")
		Eventually(func(g Gomega) {
			key := types.NamespacedName{Name: bmh.Name, Namespace: bmh.Namespace}
			var torndown metal3api.BareMetalHost
			g.Expect(clusterProxy.GetClient().Get(ctx, key, &torndown)).To(Succeed())
			g.Expect(torndown.Annotations).NotTo(HaveKey(installStartedAnnotation))
		}, e2eConfig.GetIntervals(specName, "wait-deprovisioning")...).Should(Succeed())
	})
