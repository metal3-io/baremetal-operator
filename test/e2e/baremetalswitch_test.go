...
// createCredentialSecret creates a Kubernetes secret for switch credentials.
// The secret does not need pre-labeling because the controller uses
// SecretManager.AcquireSecret which finds unlabeled secrets via the API
// reader and labels them automatically for the filtered cache.
func createCredentialSecret(ctx context.Context, c client.Client, namespace, name string, data map[string]string) {
...
// createEmptySecret creates an empty secret for the controller to update.
// The secret does not need pre-labeling because the controller uses
// SecretManager.ObtainSecret which finds unlabeled secrets via the API
// reader and labels them automatically for the filtered cache.
func createEmptySecret(ctx context.Context, c client.Client, namespace, name string) {
	secret := corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: namespace,
	...
	if err != nil {
		Expect(k8serrors.IsAlreadyExists(err)).To(BeTrue(), fmt.Sprintf("Failed to create empty secret '%s/%s': %v", namespace, name, err))
	}
}
...
var _ = Describe("baremetalswitch", Serial, Label("required", "baremetalswitch"), func() {
	...
	BeforeEach(func() {
		...
		createEmptySecret(ctx, clusterProxy.GetClient(), namespace.Name, configSecretName)
		createEmptySecret(ctx, clusterProxy.GetClient(), namespace.Name, credentialSecretName)
	})
	...
	It("should manage switch config from BareMetalSwitch resources", func() {
		...
		createCredentialSecret(ctx, c, namespace.Name, "switch1-creds", map[string]string{
			"username": "admin",
			"password": "secret123",
		})
		...
		By("verifying the Reconciled condition is True on the first switch")
		Eventually(func(g Gomega) {
			sw := &metal3api.BareMetalSwitch{}
			g.Expect(c.Get(ctx, client.ObjectKeyFromObject(switch1), sw)).To(Succeed())
			g.Expect(sw.Status.Conditions).To(ContainCondition(string(metal3api.SwitchConditionValid), metav1.ConditionTrue))
		}, e2eConfig.GetIntervals(specName, "wait-secret-updated")...).Should(Succeed())

		By("creating a credential secret for switch-2")
		createCredentialSecret(ctx, c, namespace.Name, "switch2-creds", map[string]string{
			"username": "root",
			"password": "p@ssw0rd",
		})
		...
		createCredentialSecret(ctx, c, namespace.Name, "pw-switch-creds", map[string]string{
			"username": "admin",
			"password": "secret123",
		})
		...
		createCredentialSecret(ctx, c, namespace.Name, "pk-switch-creds", map[string]string{
			"username":       "sshuser",
		...
		createCredentialSecret(ctx, c, namespace.Name, "rotating-creds", map[string]string{
			"username": "admin",
		...
		By("verifying the Reconciled condition is False with CredentialError")
		Eventually(func(g Gomega) {
			sw := &metal3api.BareMetalSwitch{}
			g.Expect(c.Get(ctx, client.ObjectKeyFromObject(missingCredsSwitch), sw)).To(Succeed())
			g.Expect(sw.Status.Conditions).To(ContainCondition(string(metal3api.SwitchConditionValid), metav1.ConditionFalse))
		}, e2eConfig.GetIntervals(specName, "wait-secret-updated")...).Should(Succeed())

		By("creating the missing credential secret")
		createCredentialSecret(ctx, c, namespace.Name, "nonexistent-creds", map[string]string{
			"username": "admin",
			"password": "secret123",
		})
		...
		By("verifying the Reconciled condition flips to True after secret is created")
		Eventually(func(g Gomega) {
			sw := &metal3api.BareMetalSwitch{}
			g.Expect(c.Get(ctx, client.ObjectKeyFromObject(missingCredsSwitch), sw)).To(Succeed())
			g.Expect(sw.Status.Conditions).To(ContainCondition(string(metal3api.SwitchConditionValid), metav1.ConditionTrue))
		...
		By("creating a credential secret with missing password and ssh-privatekey keys")
		createCredentialSecret(ctx, c, namespace.Name, "bad-creds", map[string]string{
			"username": "admin",
		})
		badCredsSwitch := &metal3api.BareMetalSwitch{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "bad-creds-switch",
				Namespace: namespace.Name,
			},
			Spec: metal3api.BareMetalSwitchSpec{
				Address:    "192.168.1.1",
		...
		Expect(c.Create(ctx, badCredsSwitch)).To(Succeed())

		By("verifying the Reconciled condition is False with CredentialError for misconfigured secret")
		Eventually(func(g Gomega) {
			sw := &metal3api.BareMetalSwitch{}
			g.Expect(c.Get(ctx, client.ObjectKeyFromObject(badCredsSwitch), sw)).To(Succeed())
			g.Expect(sw.Status.Conditions).To(ContainCondition(string(metal3api.SwitchConditionValid), metav1.ConditionFalse))
		}, e2eConfig.GetIntervals(specName, "wait-secret-updated")...).Should(Succeed())

		By("fixing the misconfigured credential secret by adding the password key")
		badSecret := &corev1.Secret{}
		badSecretKey := types.NamespacedName{Namespace: namespace.Name, Name: "bad-creds"}
		Expect(c.Get(ctx, badSecretKey, badSecret)).To(Succeed())
		badSecret.Data["password"] = []byte("fixed-password")
		Expect(c.Update(ctx, badSecret)).To(Succeed())

		By("verifying the Owns() watch triggers reconciliation and config is populated")
		Eventually(func(g Gomega) {
			config := getSwitchConfig(ctx, c, namespace.Name)
			g.Expect(config).To(ContainSubstring("[switch:bad-creds-switch]"))
			g.Expect(config).To(ContainSubstring("password=fixed-password"))
		}, e2eConfig.GetIntervals(specName, "wait-secret-updated")...).Should(Succeed())

		By("verifying the Reconciled condition flips to True after secret is fixed")
		Eventually(func(g Gomega) {
			sw := &metal3api.BareMetalSwitch{}
			g.Expect(c.Get(ctx, client.ObjectKeyFromObject(badCredsSwitch), sw)).To(Succeed())
			g.Expect(sw.Status.Conditions).To(ContainCondition(string(metal3api.SwitchConditionValid), metav1.ConditionTrue))
		...
		By("cleaning up misconfigured creds test switch")
		Expect(c.Delete(ctx, badCredsSwitch)).To(Succeed())
		Eventually(func(g Gomega) {
			config := getSwitchConfig(ctx, c, namespace.Name)
			g.Expect(config).To(Equal(switchConfigHeader))
		}, e2eConfig.GetIntervals(specName, "wait-secret-updated")...).Should(Succeed())
		...
		for _, name := range []string{"a-creds", "b-creds", "c-creds"} {
			createCredentialSecret(ctx, c, namespace.Name, name, map[string]string{
				"username": "admin",
				"password": "pass-" + name,
			})
		}
		...
		for i, name := range []string{"switch-a", "switch-b", "switch-c"} {
			switches[i] = &metal3api.BareMetalSwitch{
				...
				Spec: metal3api.BareMetalSwitchSpec{
					...
					DeviceType: "netmiko_cisco_ios",
					Credentials: &corev1.SecretReference{
						Name: fmt.Sprintf("%c-creds", 'a'+rune(i)),
					},
				},
			}
	...
	It("should handle optional switch fields", func() {
		c := clusterProxy.GetClient()

		By("creating a credential secret for optional fields test")
		createCredentialSecret(ctx, c, namespace.Name, "optional-creds", map[string]string{
			"username": "admin",
			"password": "secret123",
		})
