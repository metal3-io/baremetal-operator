...
var _ = Describe("baremetalswitch", Serial, Label("required", "baremetalswitch"), func() {
	...
	It("should manage switch config from BareMetalSwitch resources", func() {
		...
		Eventually(func(g Gomega) {
			...
			cond := meta.FindStatusCondition(sw.Status.Conditions, string(metal3api.SwitchConditionReconciled))
		...
		By("creating a credential secret with missing password key")
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(cond.Status).To(Equal(metav1.ConditionFalse))
			g.Expect(cond.Reason).To(Equal("CredentialError"))
			g.Expect(cond.Message).To(ContainSubstring("missing 'password' key"))
		}, e2eConfig.GetIntervals(specName, "wait-secret-updated")...).Should(Succeed())
