...
func PatchBMHForProvisioning(ctx context.Context, input PatchBMHForProvisioningInput) error {
	...
	input.bmh.Spec.RootDeviceHints = &metal3api.RootDeviceHints{
		DeviceName: "/dev/vda",
	}
...
// This userdata formats /dev/vdb, mounts it, and creates a test file. Intended for testing automated cleaning of disks.
func createDiskTestUserdata(ctx context.Context, client client.Client, namespace string, secretName string) {
	userDataContent := `#!/bin/sh
