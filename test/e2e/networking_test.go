...
var _ = Describe("networking", Label("required", "networking"), func() {
	...
	It("should create and validate HostNetworkAttachment resources", func() {
		...
		toCleanup = append(toCleanup, accessHNA)
		...
		toCleanup = append(toCleanup, trunkHNA)
	...
	It("should handle BMH with switch port identity on network interfaces", func() {
		...
		toCleanup = append(toCleanup, hna)
	...
	It("should validate network interfaces and apply port configs through full lifecycle", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-lifecycle", bmcCredentialsData)
		...
		By("verifying NetworkInterfacesValid condition and AppliedNetworkAttachmentConfigs")
		Eventually(func(g Gomega) {
			retrievedBMH := &metal3api.BareMetalHost{}
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, retrievedBMH)).To(Succeed())
			...
			g.Expect(cond.Reason).To(Equal("AllInterfacesValid"))
			g.Expect(retrievedBMH.Status.AppliedNetworkAttachmentConfigs).NotTo(BeEmpty(), "AppliedNetworkAttachmentConfigs should be set")
			g.Expect(retrievedBMH.Status.AppliedNetworkAttachmentConfigs[0].SwitchPortConfig.Mode).To(Equal(metal3api.SwitchPortMode("access")))
			g.Expect(retrievedBMH.Status.AppliedNetworkAttachmentConfigs[0].SwitchPortConfig.NativeVLAN).To(Equal(100))
	...
	It("should set NetworkInterfacesValid to False for invalid interface names", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-invalid-nic", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(cond.Reason).To(Equal("InvalidInterfaceNames"))
			g.Expect(cond.Message).To(ContainSubstring("does-not-exist"))
			g.Expect(retrievedBMH.Status.AppliedNetworkAttachmentConfigs).To(BeEmpty())
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should re-apply port configs when NetworkInterfaces change on available host", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-reconfig", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		toCleanup = append(toCleanup, hna1)
		Expect(c.Create(ctx, hna2)).To(Succeed())
		toCleanup = append(toCleanup, hna2)
		...
		By("verifying initial AppliedNetworkAttachmentConfigs")
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).To(HaveLen(1))
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs[0].SwitchPortConfig.NativeVLAN).To(Equal(100))
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
		...
		By("waiting for AppliedNetworkAttachmentConfigs to reflect the new VLAN")
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.Provisioning.State).To(Equal(metal3api.StateAvailable))
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).To(HaveLen(1))
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs[0].SwitchPortConfig.NativeVLAN).To(Equal(200))
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
		...
		By("waiting for AppliedNetworkAttachmentConfigs to be cleared and condition removed")
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.Provisioning.State).To(Equal(metal3api.StateAvailable))
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).To(BeEmpty())
	...
	It("should apply switchport configuration to Ironic ports", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-ironic-ports", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		By("waiting for the network configuration to be applied")
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: bmh.Namespace}, bmh)).To(Succeed())
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).NotTo(BeEmpty())
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should apply port configs when NetworkInterfaces are added to an available host", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-add-ni", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		Expect(retry.RetryOnConflict(retry.DefaultBackoff, func() error {
			if err := c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh); err != nil {
				return err
			}
			bmh.Spec.NetworkInterfaces = []metal3api.NetworkInterface{
				{
		...
		By("waiting for AppliedNetworkAttachmentConfigs to be populated after Preparing cycle")
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.Provisioning.State).To(Equal(metal3api.StateAvailable))
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).NotTo(BeEmpty())
	...
	It("should maintain port configs during deprovisioning", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-lldp", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).To(BeEmpty(),
				"AppliedNetworkAttachmentConfigs should be cleared after NI removal")
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should clear port configs only after deprovisioning completes when NI removed atomically", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-atomic-deprov", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		By("verifying AppliedNetworkAttachmentConfigs still present during deprovisioning")
		Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
		Expect(bmh.Status.AppliedNetworkAttachmentConfigs).NotTo(BeEmpty(),
			"AppliedNetworkAttachmentConfigs should be preserved during deprovisioning")
		...
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).To(BeEmpty(),
				"AppliedNetworkAttachmentConfigs should be cleared after deprovisioning")
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should preserve port configs when NI removed separately during deprovisioning", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-ni-during-deprov", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		By("verifying AppliedNetworkAttachmentConfigs still present during deprovisioning after NI removal")
		Consistently(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			if bmh.Status.Provisioning.State == metal3api.StateDeprovisioning {
				g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).NotTo(BeEmpty(),
					"AppliedNetworkAttachmentConfigs should be preserved during deprovisioning even after NI removal")
			}
		...
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).To(BeEmpty(),
				"AppliedNetworkAttachmentConfigs should be cleared after deprovisioning completes")
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should block provisioning when network interfaces reference missing HNA", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-block-prov", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(cond).NotTo(BeNil(), "NetworkInterfacesValid condition should exist")
			g.Expect(cond.Status).To(Equal(metav1.ConditionFalse))
			g.Expect(cond.Reason).To(Equal("AttachmentNotFound"))
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should recover validation when missing HNA is created", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-recover", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
		...
		Eventually(func(g Gomega) {
			g.Expect(c.Get(ctx, types.NamespacedName{Name: bmh.Name, Namespace: namespace.Name}, bmh)).To(Succeed())
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs).NotTo(BeEmpty(),
				"AppliedNetworkAttachmentConfigs should be set after recovery")
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs[0].SwitchPortConfig.Mode).To(Equal(metal3api.SwitchPortMode("access")))
			g.Expect(bmh.Status.AppliedNetworkAttachmentConfigs[0].SwitchPortConfig.NativeVLAN).To(Equal(100))
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
	...
	It("should preserve LLDP data on Ironic ports", func() {
		...
		secret := CreateSecret(ctx, c, namespace.Name, "bmc-creds-deprov", bmcCredentialsData)
		toCleanup = append(toCleanup, secret)
