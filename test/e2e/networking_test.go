...
var _ = Describe("networking", Label("required", "networking"), func() {
	...
	It("should validate network interfaces and apply port configs through full lifecycle", func() {
		...
		By("verifying NetworkInterfacesValid condition and AppliedPortConfigs")
		Eventually(func(g Gomega) {
			...
			g.Expect(retrievedBMH.Status.AppliedPortConfigs).NotTo(BeEmpty(), "AppliedPortConfigs should be set")
			g.Expect(retrievedBMH.Status.AppliedPortConfigs[0].SwitchPortConfig.Mode).To(Equal(metal3api.SwitchPortMode("access")))
			g.Expect(retrievedBMH.Status.AppliedPortConfigs[0].SwitchPortConfig.NativeVLAN).To(Equal(100))
	...
	It("should set NetworkInterfacesValid to False for invalid interface names", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(retrievedBMH.Status.AppliedPortConfigs).To(BeEmpty())
		}, e2eConfig.GetIntervals("default", "wait-available")...).Should(Succeed())
