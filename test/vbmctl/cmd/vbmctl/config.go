				...
				if sushyConfig != nil {
					...
					fmt.Printf("  Config File: %s\n  Listening: %s\n",
						func() string {
							...
							return sushyConfig.ConfigFile
						}(),
						func() string {
							if sushyConfig.ListenPort == 0 || sushyConfig.ListenAddress == "" {
								if sushyConfig.ConfigFile != "" {
									return "(configuration file in use)"
								}
								return "(not specified)"
							}
							return sushyConfig.ListenAddress + ":" + strconv.FormatUint(uint64(sushyConfig.ListenPort), 10)
						}())
				}
			}
