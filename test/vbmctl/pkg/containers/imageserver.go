...
func CreateImageServerInstance(ctx context.Context, cfg *vbmctlapi.ImageServerConfig) error {
	...
	for _, p := range cfg.ExtraPorts {
		...
		if _, exists := portBindings[extraContainerPort]; exists {
			return fmt.Errorf("extra port %d is already in use by the image server or another extra port", p.ContainerPort)
		}
		if _, exists := hostPorts[p.HostPort]; exists {
			return fmt.Errorf("extra host port %d is already in use by the image server or another extra port", p.HostPort)
		}
