...
func ConnectWithVeth(_ context.Context, network1 string, network2 string, vethpeer1 string, vethpeer2 string) error {
	...
	if err := netlink.LinkSetMaster(veth1, link1); err != nil {
		return fmt.Errorf("failed to set link %s for veth %s: %w", network1, vethpeer1, err)
	}
	if err := netlink.LinkSetMaster(veth2, link2); err != nil {
		return fmt.Errorf("failed to set link %s for veth %s: %w", network2, vethpeer2, err)
	}
	return nil
}
