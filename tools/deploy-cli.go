...
// execCommand executes commands and captures their output. If a dir is specified, the command runs in that directory.
func execCommand(dir, command string, args ...string) {
	cmd := exec.Command(command, args...)
	// Set the working directory only if dir is specified and not an empty string
	if dir != "" {
		cmd.Dir = dir
	}
...
func setupIronicDeployment(deployBasicAuthFlag, deployTLSFlag, deployKeepalivedFlag, deployMariadbFlag bool, tempIronicOverlay, kustomizePath string) {
	execCommand(tempIronicOverlay, kustomizePath, "create", "--resources=../../../config/namespace", "--namespace=baremetal-operator-system", "--nameprefix=baremetal-operator-")

	if deployBasicAuthFlag {
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "secret", "ironic-htpasswd", "--from-env-file=ironic-htpasswd")
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "secret", "ironic-auth-config", "--from-file=auth-config=ironic-auth-config")
		...
		if deployTLSFlag {
			...
			execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "resource", "../../overlays/basic-auth_tls")
		} else {
			execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "resource", "../../base")
			execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "component", "../../components/basic-auth")
		}
	} else if deployTLSFlag {
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "component", "../../components/tls")
	}

	if deployKeepalivedFlag {
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "component", "../../components/keepalived")
	}

	if deployMariadbFlag {
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "component", "../../components/mariadb")
	}
}
...
func setupBMODeployment(deployBasicAuthFlag, deployTLSFlag bool, tempBMOOverlay, kustomizePath string) {
	execCommand(tempBMOOverlay, kustomizePath, "create", "--resources=../../base,../../namespace", "--namespace=baremetal-operator-system")

	if deployBasicAuthFlag {
		execCommand(tempBMOOverlay, kustomizePath, "edit", "add", "component", "../../components/basic-auth")
		execCommand(tempBMOOverlay, kustomizePath, "edit", "add", "secret", "ironic-credentials", "--from-file=username=ironic-username", "--from-file=password=ironic-password")
	}

	if deployTLSFlag {
		execCommand(tempBMOOverlay, kustomizePath, "edit", "add", "component", "../../components/tls")
	}
}
...
func deployBMO(tempBMOOverlay, scriptDir, kustomizePath string) error {
	...
	execCommand(tempBMOOverlay, kustomizePath, "edit", "add", "configmap", "ironic", "--behavior=create", "--from-env-file=ironic.env")
...
func deployIronic(tempIronicOverlay, scriptDir, kustomizePath string, deployKeepalivedFlag bool) error {
	...
	if deployKeepalivedFlag {
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "configmap", "ironic-bmo-configmap", "--behavior=replace", "--from-env-file=ironic_bmo_configmap.env")
	} else {
		execCommand(tempIronicOverlay, kustomizePath, "edit", "add", "configmap", "ironic-bmo-configmap", "--behavior=create", "--from-env-file=ironic_bmo_configmap.env")
	}
