...
CONTAINER_PULL_IMAGES=${CONTAINER_PULL_IMAGES:-"true"}
HTTP_PORT="6180"
...
if [ -f "$IRONIC_EXTRA_VARS" ]; then
      sudo cat "$IRONIC_EXTRA_VARS" | sudo tee -a "${IRONIC_DATA_DIR}/ironic-vars.env"
fi
...
if [ "$CONTAINER_PULL_IMAGES" == "true" ]; then
